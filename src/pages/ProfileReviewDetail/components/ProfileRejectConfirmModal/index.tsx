...
import type { InstructorProfileRejectReasonCategory } from '@/types/profile';
...
const ProfileRejectConfirmModal = ({ tempInstructorId }: ProfileRejectConfirmModalProps) => {
	...
	const [selectedRejectReason, setSelectedRejectReason] = useState<InstructorProfileRejectReasonCategory | null>(null);
	...
	const profileRejectMutation = useMutation({
		...
		mutationFn: (request: { category: InstructorProfileRejectReasonCategory; detail: string }) =>
			postRejectReview(tempInstructorId, request),
		onSuccess: () => {
			closeModal();
	...
	const handleRejectConfirm = () => {
		profileRejectMutation.mutate({
			category: selectedRejectReason as InstructorProfileRejectReasonCategory,
	...
	const dropdownItems: MenuProps['items'] = REJECT_REASON_DROPDOWN_ITEMS.map((item) => ({
		key: item.key,
		label: item.label,
		onClick: () => setSelectedRejectReason(item.key as InstructorProfileRejectReasonCategory),
	}));
